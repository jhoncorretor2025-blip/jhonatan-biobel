...
const parseImportedDate = (val, fallbackStr = '') => {
  if (!val) return fallbackStr || '2026-06-18';
  ...
  const dmyComplete = s.match(/^(\d{1,2})[./-](\d{1,2})[./-](\d{2,4})/);
  if (dmyComplete) {
    let [_, d, m, y] = dmyComplete;
    if (y.length === 2) y = `20${y}`;
    return `${y}-${m.padStart(2, '0')}-${d.padStart(2, '0')}`;
  }
  const d = new Date(s);
  if (!isNaN(d.getTime())) return d.toISOString().split('T')[0];
  return fallbackStr || '2026-06-18';
};
...
fetch(publicUrl)
  ...
  .then(buffer => {
    ...
    const sheetName = '18.06';
    const sheet = workbook.Sheets[sheetName];
    const rawData = XLSX.utils.sheet_to_json(sheet, { header: 1 });
    
    let sheetDate = '2026-06-18';
    ...
    console.log('Header index found:', headerIdx);
    console.log('Column Map:', colMap);

    let allImportedSales = [];
    if (headerIdx !== -1) {
      const pMethods = [
      ...
      for (let i = headerIdx + 1; i < rawData.length; i++) {
        ...
        if (String(row[0] || '').toLowerCase().includes('total')) {
          console.log(`Row ${i+1} skipped because it has "total" in col 0: "${row[0]}"`);
          continue;
        }
    ...
    console.log(`Parsed ${allImportedSales.length} sales:`);
    console.log(allImportedSales);
  });
