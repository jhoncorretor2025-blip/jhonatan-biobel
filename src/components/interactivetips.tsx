        ...
        <div className="space-y-4 text-left">
          ...
          {inactiveCustomersData.list.length > 0 && (
            <div className="space-y-3 border-t border-slate-100 dark:border-slate-800 pt-3">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Clientes ausentes & Artes de abordagem:</p>
              <div className="space-y-3">
                {inactiveCustomersData.list.map(c => {
                  const msgTemplate = `Oi, ${c.name}! Tudo bem? ✨\n\nSentimos muito a sua falta aqui na BIOBEL! 🌸 Faz um tempinho que você não vem se cuidar conosco...\n\nPara matar a saudade e te dar aquele carinho especial, preparamos um presente exclusivo para você: *10% de desconto* na sua próxima visita ou compra de produtos! 🎁💖\n\nQue tal aproveitar essa semana para tirar um momento para você?\n\nMe conta, qual dia fica melhor para agendarmos seu horário? 🥰`;
                  const waUrl = `https://api.whatsapp.com/send?phone=55${c.phone.replace(/\D/g, '')}&text=${encodeURIComponent(msgTemplate)}`;
                  return (
                    <div key={c.id} className="p-4 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800/60 pb-2">
                        <div>
                          <p className="text-xs font-black uppercase text-slate-800 dark:text-slate-200">{c.name}</p>
                          <p className="text-[10px] font-semibold text-rose-500">{c.daysAway} dias sem comprar</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <button 
                            onClick={() => handleCopyText(msgTemplate, `crm-${c.id}`)}
                            className="px-2.5 py-1.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-750 rounded-xl border border-slate-200 dark:border-slate-700 text-[10px] font-bold uppercase transition flex items-center gap-1 cursor-pointer"
                          >
                            <Copy size={11} />
                            {copiedText === `crm-${c.id}` ? "Copiado! ✓" : "Copiar Arte"}
                          </button>
                          <a 
                            href={waUrl} 
                            target="_blank" 
                            rel="noreferrer" 
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-[10px] rounded-xl tracking-wide uppercase transition-all shadow-sm"
                          >
                      ...
                      {/* Simulated WhatsApp chat bubble for the marketing Art */}
                      <div className="bg-[#DCF8C6] dark:bg-[#054740]/25 text-slate-800 dark:text-slate-200 p-3 rounded-2xl rounded-tr-none text-[11px] font-medium leading-relaxed max-w-[95%] shadow-sm border border-emerald-100/40 dark:border-emerald-900/10 relative ml-auto font-sans">
                        <div className="whitespace-pre-line">{msgTemplate}</div>
                        <span className="text-[8px] text-emerald-700 dark:text-emerald-400 absolute bottom-1 right-2 font-black">BIOBEL • Relacionamento</span>
                      </div>
                    </div>
                  );
                })}
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4 text-left">
          ...
          {/* Upgraded "Arte do Roteiro" section */}
          {(() => {
            const salesScriptText = `Meninas, vamos bombar as vendas de cosméticos hoje! 🚀✨\n\nAbordagem prática de Cross-Selling:\n\n*Ao finalizar um serviço ou venda de produto:* \n"Olá, [Nome]! Sabia que hoje, levando a Máscara de Tratamento junto com esse shampoo, ela sai com *20% de DESCONTO*? É perfeito para manter o efeito do tratamento de salão em casa!" 🥰🧴\n\n*Oferecendo Kit/Combo:* \n"Temos também esse kit de alta performance com a necessaire exclusiva da marca por apenas R$ 19,90 adicionais no combo!"\n\nBoas vendas! Vamos bater nossa meta semanal de faturamento! 💪🏆`;
            return (
              <div className="space-y-2.5 border-t border-slate-100 dark:border-slate-800 pt-3.5">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Arte do Roteiro de Abordagem Comercial:</span>
                  <button 
                    onClick={() => handleCopyText(salesScriptText, 'roteiro_venda')}
                    className="px-2.5 py-1.5 bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 border border-blue-100/40 dark:border-blue-900/30 rounded-xl text-[10px] font-bold uppercase transition flex items-center gap-1 cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40"
                  >
                    <Copy size={11} />
                    {copiedText === 'roteiro_venda' ? "Copiado! ✓" : "Copiar Roteiro"}
                  </button>
                </div>
                <div className="bg-blue-50/40 dark:bg-blue-950/15 text-slate-800 dark:text-slate-200 p-3.5 rounded-3xl border border-blue-100/30 dark:border-blue-900/10 text-[11px] leading-relaxed relative font-sans">
                  <div className="font-bold text-blue-700 dark:text-blue-400 mb-1.5 flex items-center gap-1 uppercase text-[9px] tracking-widest">
                    <Sparkles size={12} className="text-blue-500" /> Abordagem de Alta Performance Recomendada
                  </div>
                  <div className="whitespace-pre-line font-medium">{salesScriptText}</div>
                </div>
              </div>
            );
          })()}
      ...
      content: (
        <div className="space-y-4 text-left">
          ...
          {upcomingBirthdays.celebrators.length > 0 ? (
            <div className="space-y-3 border-t border-slate-100 dark:border-slate-800 pt-3">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Aniversariantes do Mês & Artes de Parabéns:</p>
              <div className="space-y-3">
                {upcomingBirthdays.celebrators.slice(0, 4).map(c => {
                  const bMsg = `Parabéns, ${c.name}! 🎉✨\n\nNós da BIOBEL desejamos um feliz aniversário, muita saúde, amor e sucesso! 🌸\n\nPara celebrar este dia tão especial, preparamos um presente exclusivo para você: *R$ 20,00 de presente* para usar em qualquer compra de produto ou serviço esta semana! 🎁💖\n\nQue tal aproveitar para se cuidar e renovar sua beleza? 🥰🧴\n\nPosso agendar um momento especial para você? Responda aqui para reservar seu horário!`;
                  ...
                  return (
                    <div key={c.id} className="p-4 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800/60 pb-2">
                        <div>
                          <p className="text-xs font-black uppercase text-slate-800 dark:text-slate-200 truncate max-w-[150px]">{c.name}</p>
                          <p className="text-[10px] font-semibold text-rose-500">Aniversariante • Dia {c.formattedBirthday}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <button 
                            onClick={() => handleCopyText(bMsg, `bday-${c.id}`)}
                            className="px-2.5 py-1.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-750 rounded-xl border border-slate-200 dark:border-slate-700 text-[10px] font-bold uppercase transition flex items-center gap-1 cursor-pointer"
                          >
                            <Copy size={11} />
                            {copiedText === `bday-${c.id}` ? "Copiado! ✓" : "Copiar Arte"}
                          </button>
                          <a 
                            href={waUrl} 
                            target="_blank" 
                            rel="noreferrer"
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-500 hover:bg-rose-600 text-white font-bold text-[10px] rounded-xl tracking-wide uppercase transition-all shadow-sm cursor-pointer"
                            title="Enviar Parabéns!"
                          >
                            <Gift size={11} /> Parabenizar WhatsApp
                          </a>
                        </div>
                      </div>

                      {/* Simulated WhatsApp chat bubble for Birthday Art */}
                      <div className="bg-[#DCF8C6] dark:bg-[#054740]/25 text-slate-800 dark:text-slate-200 p-3 rounded-2xl rounded-tr-none text-[11px] font-medium leading-relaxed max-w-[95%] shadow-sm border border-emerald-100/40 dark:border-emerald-900/10 relative ml-auto font-sans">
                        <div className="whitespace-pre-line">{bMsg}</div>
                        <span className="text-[8px] text-emerald-700 dark:text-emerald-400 absolute bottom-1 right-2 font-black">BIOBEL • Celebrando</span>
                      </div>
                    </div>
                  );
                })}
